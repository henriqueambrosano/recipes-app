...
describe('RecipeInProgress page tests', () => {
  ...
  it('Testar componentes caso ja tenha não tenha nada no localStorage', async () => {
    ...
    expect(await screen.findByTestId('0-horizontal-name')).toHaveTextContent('Corba');
    expect(screen.queryByTestId('1-horizontal-image')).not.toBeInTheDocument();

    userEvent.click(FILTER_DRINK);

    expect(await screen.findByTestId('0-horizontal-name')).toBeInTheDocument();
    expect(await screen.findByTestId('0-horizontal-name')).toHaveTextContent('A1');
    expect(screen.queryByTestId('1-horizontal-image')).not.toBeInTheDocument();
    ...
    expect(await screen.findByTestId('1-horizontal-name')).toHaveTextContent('A1');
